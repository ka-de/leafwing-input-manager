...
impl ClashTestExt for App {
    fn assert_input_map_actions_eq(
        ...
        let mut input_system_state: SystemState<Query<&InputMap<Action>>> = SystemState::new(
            &mut self.world
        );
...
fn three_inputs_clash_handling() {
    ...
    app.assert_input_map_actions_eq(ClashStrategy::PressAll, [
        One,
        Two,
        OneAndTwo,
        TwoAndThree,
...
fn modifier_clash_handling() {
    ...
    app.assert_input_map_actions_eq(ClashStrategy::PrioritizeLongest, [CtrlOne, OneAndTwoAndThree]);
}
